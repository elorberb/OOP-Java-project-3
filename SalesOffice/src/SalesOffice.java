...
public class SalesOffice {
	...
	private void readFiles(String fileEvents, String fileEmployees, String fileCustomers, String fileTicketsSales) {// reads
																													// all
																													// files
	...
	private void readCustomersFile(String file) { // reads from the Customers file.
		...
		try {
			...
			while ((line = br.readLine()) != null) {// runs until line is null
				...
				newCustomer(temp); // creating new customer
			}
	...
	private void addingNewCustomerToList(int id, String name, int age, char gender, int empId) { // inserting new
																									// customer into the
																									// list.
		try {
			Customer c = new Customer(id, name, age, gender, empId);// creating new customer
			customers.add(c);// adding him to vector
			MarketingEmployee mw = (MarketingEmployee) findEmployeeByID(c.getRegisteredEmpId());// finding the emplyee
																								// who wrote the
																								// customer
			...
			System.err.println("Customer number " + id + ": Wrong gender input, can be only 'm' or 'f' ");
	...
	private void readEmployeesFile(String file) { // reads from the Employees file.
		BufferedReader br = null;
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				newEmployee(temp); // creating new Employee
			}
	...
	private void addingNewEmployeToList(int id, String name, int age, String[] temp) { // inserting new employee into
																						// the list.
		if (temp.length == 5) {
			String phone = temp[4];
			employees.add(new MarketingEmployee(id, name, age, phone));
		}
		if (temp.length == 4) {
			double saleRate = Double.parseDouble((temp[3]));
			employees.add(new SalesEmployee(id, name, age, saleRate));
		}
	}
	...
	private void readEventsFile(String file) { // reads from the Customers file.
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				String temp[] = line.split("\\t");
				newEvent(temp); // creating new event
			}
	...
	private void addingNewEventToList(String name, String[] temp, int eventId, double pricePerTicket) { // inserting new
																										// event into
																										// the list.
		try {
			events.add(new Event(name, eventId, pricePerTicket));
		} catch (NegativePriceException e) {// catching negative price input
			System.err.println("The ticket price of the event: " + name + " cannot be negative.");
		}
	}
	...
	private void readOrdersFile(String file) { // reads from the orders file.
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				String temp[] = line.split("\\t");
				newOrder(temp); // creating new order
			}
	...
	private void addingNewOrderToList(String[] temp, int customerId, int eventId, int numberOfTickets,
			boolean byPhone) { // inserting new order into the list.
		if (eventNotExist(eventId) && customerNotExist(customerId)) {
			if (temp.length == 5) {
				String url = temp[4];
	...
	private boolean eventNotExist(int eventID) { // did not found event.
		for (Event e : events) {
	...
	private boolean customerNotExist(int customerID) { // did not found customer.
	...
	private void newOrder(String[] temp) { // saving the event values into the temp arry.
	...
	private void updateCustomerInfo(Customer c, Order o, boolean byPhone, double fullOrderPrice) {// updating the
																									// customer info.
	...
	private void updateEmployeeInfo(Order o, Customer c, boolean byPhone, double fullOrderPrice) { // updating info
																									// about the
																									// employee
	...
	private void sortingEmployee() { // prints the employees report
	...
	private void sortingEvent() { // prints the event report
	...
	private void sortingCustomers() { // prints the customer report
	...
	public static Comparable<?> getMax(Vector<? extends Comparable> comparable) {// return the maximum value by
																					// comparable
